...
fn make_wall() -> std::boxed::Box<(dyn Wall + 'static)> { Box::new(EnchanedWall{})}
fn make_room() -> std::boxed::Box<(dyn Room + 'static)> { Box::new(EnchanedRoom{})}
fn make_door() -> std::boxed::Box<(dyn Door + 'static)> { Box::new(EnchanedDoor{})}
}
fn main() {
